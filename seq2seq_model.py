...
class Seq2SeqModel():
    ...
    def __init__(self, encoder_cell, decoder_cell, output_dim,
                 bidirectional=True,
                 attention=False,
                 debug=False):
        self.debug = debug
    ...
    def _init_decoder_train_connectors(self):
        ...
        with tf.name_scope('DecoderTrainFeeds'):
            ...
            self.decoder_train_inputs = tf.concat([EOS_SLICE, self.decoder_targets[:-1]], axis=0)
    ...
    def _init_decoder(self):
        with tf.variable_scope("Decoder") as scope:
            def output_fn(outputs):
                return tf.contrib.layers.fully_connected(outputs, self.output_dim, activation_fn=None, scope=scope)
    ...
    def _init_optimizer(self):
        outputs = tf.transpose(self.decoder_logits_train, [1, 0, 2])
        targets = tf.transpose(self.decoder_train_targets, [1, 0])
        #self.loss = seq2seq.sequence_loss(logits=logits, targets=targets, weights=self.loss_weights)
        self.loss = tf.losses.mean_squared_error(outputs, targets)
